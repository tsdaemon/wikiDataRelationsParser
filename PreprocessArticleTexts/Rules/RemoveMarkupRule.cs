...
namespace PreprocessArticleTexts.Rules
{
    ...
    {
        private Regex regex = new Regex(@"\&(\w+)\;|\{\{([^\}]+)\}\}|'{2,6}|[=\s]+External [lL]inks[\s=]+|[=\s]+See [aA]lso[\s=]+|[=\s]+References[\s=]+|[=\s]+Notes[\s=]+|\{\{([^\}]+)\}\}|\s\(\)|/={2,}/|<\/?\w+\s?\/?>|<\/?\w+\s?\/?|\/?\w+\s?\/?>" + "|/{?class=\"[^\"]+\"/" + "|/!?\\s?width=\"[^\"]+\"/" + "|/!?\\s?height=\"[^\"]+\"/" + "|/!?\\s?style=\"[^\"]+\"/" + "|/!?\\s?rowspan=\"[^\"]+\"/");
